//! INVDB is an embedded, page-based database engine that treats invariants as
//! first-class. Prompt 1 establishes the crate layout, public contracts, and
//! foundational types; Prompt 2 makes the on-disk format concrete.
//!
//! ## Invariants
//! - INV-1: Page size is constant for a DB file, and must match compile-time constant PAGE_SIZE.
//! - INV-2: All on-disk structures are page-aligned and page-addressable by PageId.
//! - INV-3: No silent corruption: if bytes don’t match expected format, return an error.
//! - INV-4: A DB file has a magic header and version; mismatch is an error unless explicitly opened in “compat mode” (not in Prompt 1).
//! - INV-5: Deterministic encoding: all primitive encoding used by INVDB is specified and stable.
//! - INV-6: All public APIs return Result<T, InvError>; never panic for user input or IO issues (debug_assert! is OK for internal invariants only).
//! - INV-7: Operations that mutate persistent state will be explicit and will require a mutable handle or explicit transaction context (transactions come later).
//! - INV-8: IDs are strongly typed (PageId is not u64; it is a distinct type).
//! - INV-9: Error messages are stable enough to be useful; errors carry context where appropriate.
//! - INV-10: Versioning is explicit: file format version increments must be controlled by constants.
//!
//! ## Roadmap
//! - file.rs: raw file open/create/read/write primitives
//! - page.rs: page buffer + header layout
//! - pager.rs: caching, dirty tracking, flushing
//! - btree/*: btree node layout + search + insert + split
//! - row.rs: row encoding/decoding
//! - table.rs: table interface
//! - catalog.rs: system metadata
//! - db.rs: user-level db operations
//!
//! ## Non-goals (for now)
//! - Concurrency control
//! - Write-ahead logging (WAL)
//! - SQL planner
//! - Durable SQL interface (temporary KV API exists for testing)
//!
//! ## Stability
//! The stable, supported surface of this crate is the `Db` type and the methods
//! documented on it (`create`, `open`, `flush`, `put_u64`, `get_u64`,
//! `create_table`, `insert_row`, `get_row_by_pk`, `scan_table`). All other
//! modules are internal implementation details and may change without notice.
pub mod config;
pub mod error;
pub mod types;
pub mod file;
